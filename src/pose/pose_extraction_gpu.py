...
class MoveNetExtractor:
    ...
    def __init__(self, model_name: str = 'thunder', sequence_length: int = 60):
        ...
        gpus = tf.config.list_physical_devices('GPU')
        print(f"[OK] MoveNet {model_name} loaded (input size: {self.input_size}x{self.input_size})")
...
if __name__ == "__main__":
    ...
    if os.path.exists(test_video):
        ...
        if result:
            print(f"[OK] Pose sequence shape: {result['pose_sequence'].shape}")
            print(f"[OK] Gait coords shape: {result['gait_features']['normalized_coords'].shape}")
            print(f"[OK] Joint angles shape: {result['gait_features']['joint_angles'].shape}")
    else:
